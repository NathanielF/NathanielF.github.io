...
This sidesteps the cross-world problem on the hazard scale — direct and indirect effects are read off the coefficients as they unfold.
...
| Weakness | No mediation | Untestable structural assumptions | Non-generative inference |
...
One model. Two complementary outputs — operating at **different scales**. The process question reads the macro trajectory. The difference-making question commits to micro-level structure. LOO tests whether the gap matters and Sensitivty analysis can show robustness to violations.
...
Refusing to decompose isn't caution, it's a commitment. It assumes all paths point the same way. When they don't, **aggressive simplification is misspecification from the start**.
...
Pearl: **"Well-defined, via structural equations."**. The NDE is a computation over the model, not an experiment you run.
:::
...
::: {.fragment .fade-in}
**Modularity:** Equations must be stable under intervention is the identification condition for the NDE. The hybrid model inherits this from Pearl.
:::

::: {.fragment .fade-in}
But the macro **trajectory decomposition** is a coarser estimand. It only requires that confounding is not *systematic* across the trajectory
:::

::: {.fragment .fade-in}
::: {style="color: #4ec9b0; font-size: 1.2em; font-weight: bold; margin-top: 20px;"}
- $\alpha_1(t)$ → trajectory estimand (macro). Looser identification.

- G-computation NDE → cross-world estimand (micro). Full modularity.
...
::: {.notes}
This is the sharpest philosophical move in the talk. The key insight is that identification conditions are relative to the estimand. The model produces two kinds of output, and they carry different identification burdens. The trajectory decomposition — reading alpha_1(t) and beta(t)*alpha_2(t) as the shape of the direct and indirect effects — is a macro-level estimand. It tells you which path dominates, when, and with what sign. For this estimand, you don't need modularity to hold at every tick. You need the structural relationship between mediator and hazard to be approximately right on average across the trajectory — that confounding, if present, is not systematic. The splines operationalise this: they smooth over point-wise variation, and LOO across knot counts tests whether the macro story is robust to the granularity. The g-computation NDE is a different, finer estimand. It plugs one world's mediator into another world's hazard equation, tick by tick. This requires modularity at each time step — the equation must be stable under intervention at every point. That's a stronger condition, and it's the one the potential outcomes school rightly objects to. So the honest framing is: the model gives you a macro estimand (trajectory shape) with weaker identification conditions, and a micro estimand (NDE) with stronger ones. Both are useful. The trajectory tells you where to look. The NDE tells you how much. They have different epistemic costs, and the audience should understand the distinction. On sequential unconfoundedness specifically: every causal reading of a Cox model already assumes ignorability. The DPA model adds one mediator equation, adding one more condition. That's one step beyond vanilla survival analysis, not a leap into the impossible. And the generative model makes the assumption explicit and gives diagnostics to probe it.
:::
...
- **LOO** and **Sensitivity Analysis** tests whether your answer depends on your precise model or is robust to violated assumptions
