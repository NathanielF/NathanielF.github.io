...
::: {.fragment .fade-in}
**Act I — The puzzle:** Competing causes, competing risks, and why mediation splits the schools
:::
::: {.fragment .fade-in}
**Act II — The model:** A hybrid Bayesian DPA: Pearl's structural equations + Aalen's time-varying mechanisms
:::
::: {.fragment .fade-in}
**Act III — The lesson:** The lens, not just the answer. Why the trajectory is primary — and what it costs to go beyond it.
:::
...
Two latent afflictions. Condition A stresses the heart. Condition B affects the lungs.

- A progresses to heart failure first. B never gets the chance. The patient dies.

- **Counterfactual test:** "Would the patient have survived if Condition A hadn't developed?"

    - No — B would have caused death regardless. So A **fails** the naive counterfactual test for being the cause.

- Yet A is what did the work. The endpoint counterfactual misses **which process was operative**.
...
::: {.notes}
This is the classic pre-emption structure, stated in medical terms so the competing-risks link is explicit. Condition A and Condition B are both latent. A progresses first and causes death, pre-empting B. The naive counterfactual test on A alone — "would the patient have survived if A hadn't developed?" — returns no, because B would then have caused death. So the test denies A causal credit, even though A is what actually did the work. The point is not that counterfactual reasoning is wrong: it's that a counterfactual aimed at the endpoint, with the backup cause left free to fire, is too coarse to identify which process was operative. Note the structure: you must intervene on A *alone* with B still latent — that is what makes it pre-emption rather than a total-effect contrast. Answering "which path was operative?" requires tracing the process, not just comparing endpoints — exactly what mediation analysis in survival data demands.
:::
...
:::{.column}
- The competing causes structure is a **competing risks** problem
...
::: {.notes}
The competing risks framing is the cleaner analogy. Pre-emption — Condition A pre-empting the latent Condition B — involves redundant backup causes, which is not quite the structure of mediation. But competing risks — multiple pathways to the same event, where you need to know which one operated — maps directly onto mediation analysis. In both cases, the counterfactual test on the endpoint is insufficient: the patient dies either way in competing risks; the treatment shows "modest benefit" in mediation. You need to decompose by pathway to understand what actually happened. Whether death occurs due to a mediated harmful effect or an alternative risk pathway, it is the process decomposition that tells you where to intervene. The three views on the next slide are three responses to this challenge — each placing causation in a different home. Potential outcomes tries to express mediation decomposition as nested counterfactuals and runs into trouble. Pearl embraces structural equations to make the decomposition well-defined. Aalen models the process directly, so the decomposition is algebraic rather than counterfactual.
:::
...
```{.python code-line-numbers="|1-5|6-10|14-19|20-22|23-27"}
...
[__The DPA model is Hickam operationalised__]{.blue}:
...
The trajectory decomposition trades one assumption for another: **additive structural form** instead of cross-world counterfactual independence.
:::
...
Algebraic identity, not counterfactual subtraction. Identified by *sequential within-world unconfoundedness* + **additive structural form**. A parametric commitment, but one the data can speak to directly.
:::
...
For time-evolving mediation, the trajectory is the right unit of analysis.
:::

:::: {.columns}
::: {.column width="50%"}

- The competing causes structure showed it: ask *"would the outcome have differed at the endpoint?"* and you miss which process was operative. Ask *"which path transmitted the influence, and when?"* and you get an answer you can act on.
...
::: {.column width="50%"}
![Coming Soon to a Book Store near you!](images/bookimage.png)
:::
