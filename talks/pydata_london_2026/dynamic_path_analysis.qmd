...
- A's induces heart failure. B doesn't materialize. The patient dies.

- **Endpoint counterfactual:** "Would the patient have survived without either condition?"

    - No — B would have fired regardless. The endpoint comparison can't tell you which process did the work.

- To answer *"which path was operative?"* you have to trace the process — not compare frozen worlds.
...
::: {.notes}
The point here is not that the counterfactual framework gives the wrong answer. It's that the endpoint counterfactual is answering the wrong question. "Would the target have survived without either assassin?" is a legitimate interventional query — and the answer (no) is correct. But it's silent on which process was operative, which path transmitted the influence, and which mechanism you would need to disrupt to change the outcome. That is what competing risks analysis in medicine requires: not a different theory of causation, but a finer-grained counterfactual query targeted at the path, not the endpoint. This is the motivating structure for mediation analysis in survival data.
:::
