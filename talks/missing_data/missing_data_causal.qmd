...
# The Pitch
Causal inference techniques are __required__ for Non-response in People Analytics survey data 

Bayesian imputation methods are state of the art for theory-informed imputation of missing data 
...
- The "topology" around the gaps gives us a clue
- Bayesian inference helps to imputes the probable gradients, inclines and curves of the space in the gaps conditional on the observed data.
...
We can try to recover __[ignorable missing-ness]{.blue}__ i.e moving to MAR from MNAR conditional on group specific fixed effects.
...
## Feature Selection is not Causal Modelling{.smaller}
...
## SOTA: Bayesian Probabilistic Imputation.
...
- Imputed treatment effect distributions need not be specified simply as a linear combination of observables. 
    - __[Non-parametric methods]{.blue}__ such as BART and Dirichlet Process Mixtures can also be used. 
