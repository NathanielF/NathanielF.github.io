...
categories: [bayesian, missing data, causal inferenxe]
...
# The Pitch
Non-response in HR survey data is an invitation to causal analysis of working conditions. 

Bayesian imputation of missing data is state of the art for imputation of missing data.
...
::: {.fragment .fade-in}
- Under MNAR there __does not exist__ in general consistent estimators of $Y.$ 
:::

::: {.fragment .fade-in}
- We cannot using data distinguish cases which are MNAR V (MCAR or MAR). 

- We are __[required]{.blue}__ to make assumptions about the nature of the missing-ness
...
```{.python code-line-numbers="|5-5|37-37|49-51|67-69"}
...
```{.python code-line-numbers="|13-16|37-43|55-61|62-66|68-72"}
...
```{.python code-line-numbers="|10|11|18|20-24"}
...
## Feature Selection is not Causal Modelling
...
::: {.fragment .fade-in}
- Naively optimising for some in-sample predictive benchmark does not protect your model from out-of-sample distribution shift.
:::
...
## Counterfactuals as Imputation.

- The Fundamental Problem of Causal Inference is a Missing data problem. 
...
- Imputed treatment effect distributions need not be specified simply as a linear combination of observables. 
    - __[Non-parametric methods]{.blue}__ such as BART and Dirichlet Process Mixtures can be used. 