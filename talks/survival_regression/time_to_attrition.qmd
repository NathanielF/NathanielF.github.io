...
Concepts that defy easy panoptic survey prohibit easy action e.g. climate change.
...
$$ \underbrace{p( \beta | y)}_{\text{posterior draws}} = \frac{p(\mathbb{\beta})p(y | \beta)}{\int_{i}^{n} p(y | \mathbf{\beta_{i}})p(\mathbf{\beta_{i}}) } $$
:::
