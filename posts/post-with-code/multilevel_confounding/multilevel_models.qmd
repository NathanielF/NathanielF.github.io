...
- Estimation Strategies and Group Effects
    - We go through a paired down example of group-confounding and a range of estimation strategies aiming to account for group effect bias.
- Nested Groups, Hierarchies and Fixed Effects
    - We work through a real example of three-level group nesting; pupils in classes, in schools. 
- Two Way Fixed Effects and Temporal Confounding
    - We branch off into examining a canonical estimation strategy used in DiD designs. We show how it works and how to specify these regression designs using Fixed Effects and Mundlak devices.
- Issues with TWFE and Richly Parameterised Linear Models
    - We examine the problems arising for the TWFE estimator in the case of staggered roll-outs. We also see how to address the issue with Mundlak devices. 
...
This is a bernoulli outcome with group level confounds. If we model this relationship the confounding effects will bias naive parameter estimates on the covariates $X$, $Z$. We will see different results as we explore different ways of parameterising the relationships, and the trade-offs for model specifications become clearer.
...
This method posits a view that there is a shared underlying process generating each instance of group-behaviour i.e. the reliased values of the outcome within each group. We replace the indivdual fixed effects indicator columns with additional parameters modifying the intercept term.
...
Note how the blue line represents a cohort that never undergoes a treatment and is maintained as a coherent control group throughout the sequence even though we have two other cohorts. It's also worth calling out that we have different patterns in the treatment effects across the cohorts. 
...
Here we see the importance of robustness tests in estimation techniques. In the above plot we see something like the correct pattern emerging in expectation, but absurd estimates occuring on individual cases. Whereas the bottom appears much more consistent across multiple draws from the data generating process. We can also compare these models on standard adequacy measures. 
...
```{python}
model_twfe2.predict(idata_twfe2, kind='pps')
model_twfe.predict(idata_twfe, kind='pps')
fig, axs = plt.subplots(2, 1, figsize=(10, 12))
axs = axs.flatten()
az.plot_ppc(idata_twfe2, ax=axs[0])
axs[0].set_title("PPC checks for Mundlak State Cohort Interactions")
