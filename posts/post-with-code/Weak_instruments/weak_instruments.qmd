...
mod_4, mod_4_1 = make_2sls('logwage ~ 1 + edu_predicted', 'edu ~ C(qob)', iv_df)
mod_5, mod_5_1 = make_2sls('logwage ~ 1 + edu_predicted + black', 'edu ~ C(qob)', iv_df)
mod_6, mod_6_1 = make_2sls('logwage ~ 1 + edu_predicted + black + C(state)', 'edu ~ C(qob)', iv_df)
mod_7, mod_7_1 = make_2sls('logwage ~ 1 + edu_predicted + black + C(state) + C(yob)', 'edu ~ C(qob)', iv_df)

Stargazer([mod_4, mod_5, mod_6, mod_7])    
...
df_sample = iv_df.sample(1000, random_state=200)
...
coords = {'instruments': range(instrument_preds.shape[1]), 
         'covariates': range(outcome_preds.shape[1])}
...
```{python}
import arviz as az
az.summary(idata, var_names=['intercept_y', 'intercept_t', 'beta_t', 'beta_x',])
...
az.plot_pair(idata, var_names=['intercept_y', 'intercept_t', 'beta_t', 'beta_x'], divergences=True)
