...
We want now to measure the efficacy of the treatment as a function of each time point while controlling for other factors.
...
We can see here how the temporal points before the introduction of Roe show an effect on the interaction coefficeint. Which rises towards 0 after Roe just so long as we control for the independent effects of extra covariates. 
...
The benefit to allowing for a placebo factor is to test how the effect should remain pretty static in the group for which the treatment is unlikey to have an effect. This strengthens the claim that the parrallel trends assumption holds. 
...
This type of placebo design can be specified using a complex regression equation relying on interaction terms as before with the simpler DiD design, but now we also include interactions for the placebo group. 
