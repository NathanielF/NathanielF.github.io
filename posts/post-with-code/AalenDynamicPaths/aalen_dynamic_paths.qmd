...
Our discretisation of time into bins provides a natural scaffold for working with these cumulative effects. Instead of letting each time bin jump independently—which produces those jagged dpasurv plots—we smooth the path with splines
...
The payoff is substantial: Dynamic path analysis shows its value when effects attenuate or strengthen over time e.g. drug effectiveness that fades, policies with delayed impacts, interventions with "honeymoon periods". Without temporal decomposition, the treatment might appear only modestly effective—or even neutral. Our Bayesian model successfully replicates the directional trends from the `dpasurv` R package, while providing richer uncertainty quantification through full posterior distributions rather than bootstrap confidence intervals.
