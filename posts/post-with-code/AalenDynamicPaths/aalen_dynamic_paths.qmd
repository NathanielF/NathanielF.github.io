...
def make_model(data, basis, sample=True, observed=True): 
    ...
    with pm.Model(coords=coords) as aalen_dpa_model:
        ...
        log_lambda_t = (alpha_0_t[b]
                        ...
                        + alpha_2_t[b] * med_lag  # mediator effect (lagged for temporal ordering)
                        + beta_low  * I_low
                        + beta_high * I_high
        )
...
def get_marginal_survival(idata_pp, df_long, n_bins):
    """
    G-formula survival projection. Handles event and censored subjects differently:
    - Event subjects: survival drops to 0 at the event time (absorbing state).
    - Censored subjects: last observed hazard is carried forward as a
      constant-hazard extrapolation into unobserved future bins.
    """
    ...
    # Track which subjects had the terminal event (absorbing state)
    subject_had_event = np.zeros(n_subs, dtype=bool)
    subject_event_bin = np.full(n_subs, n_bins, dtype=int)

    for i, sub_id in enumerate(subjects):
        sub_mask = (df_long['subject'] == sub_id).values
        sub_data = df_long.loc[sub_mask]
        sub_bins = sub_data['bin_idx'].values
        ...
        # 2. Check if this subject experienced the terminal event
        had_event = sub_data['event'].sum() > 0
        subject_had_event[i] = had_event
        last_bin = sub_bins.max()

        if had_event:
            # Record the event bin — survival will be forced to 0 after this
            subject_event_bin[i] = last_bin
        elif last_bin < n_bins - 1:
            # Censored: carry forward last observed hazard
            last_hazard = actual_hazards[:, -1][:, np.newaxis]
    ...
    # 4. For subjects who had the event, force S(t) = 0 after event time
    for i in range(n_subs):
        if subject_had_event[i]:
            surv_curves[:, i, subject_event_bin[i]:] = 0.0
...
It's very important to understand that this counterfactual contrast aims at symmetry. Our focus is the causal estimand and we want to distill the effect of the treatment on the population. By "carrying forward" the last observed hazard for censored individuals, we maintained a stable population denominator and avoided immortal time bias. We know that most people eventually suffer the terminal event, and this g-computation approach ensures we compare the same people against themselves.

The symmetry here deserves emphasis. The process model is built from Aalen's perspective: time-varying coefficients, additive hazard paths, a joint mediator-hazard system that tracks how causation unfolds. But the g-computation step converts this processual machinery into a clean interventionist estimand. The same model, with the same assumptions, generates both counterfactual survival curves for the same population. Modeling choices like the softplus link or the hazard extrapolation for censored subjects affect both worlds equally. What survives in the difference is the causal signal. The process model tells you *why* the treatment works and where it is being undermined. The counterfactual contrast tells you *that* it works, with a defensible number attached. The full Bayesian implementation is what makes both available simultaneously.
