...
"""
Complete IRT Analysis for Condorcet Jury Theorem Breakdown
Single file - ready to run

DIAGNOSTIC FOCUS: The issue is that posterior predictive votes may not be
respecting the known ground truth. We need to carefully check how votes
are being generated in the posterior predictive.
"""
...
def simulate_irt_data(n_cases, n_jurors, 
                      true_mu_theta=0.5,   # Shift this > 0 for competence
                      true_sigma_theta=1.0, # Skill variance
                      true_sigma_delta=0.5, # Case difficulty variance
                      block_id=None, 
                      true_sigma_block=0.5,
                      treatment=None,
                      true_tau=0.8,         # Effect of "training" or "distraction"
                      seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    # Juror abilities: Center around mu_theta to ensure baseline competence > 50%
    theta = np.random.normal(true_mu_theta, true_sigma_theta, n_jurors)
    ...
    if block_id is not None:
        n_blocks = len(np.unique(block_id))
        block_effect = np.random.normal(0, true_sigma_block, n_blocks)
        theta = theta + block_effect[block_id]
    
    if treatment is not None:
        # Suggestion: use + for improvement, - for "breakdown"
        theta = theta + true_tau * treatment
    
    votes = np.zeros((n_cases, n_jurors), dtype=int)
    for i in range(n_cases):
        # IRT Probability: logit(p) = Skill - Difficulty
        logit_p_correct = theta - delta[i]
        p_correct = 1 / (1 + np.exp(-logit_p_correct))
        
        # If state is 1, vote 1 with p_correct. If state is 0, vote 0 with p_correct.
        for j in range(n_jurors):
            if true_states[i] == 1:
    ...
    print(f"Generated IRT Data (Avg p_correct logic):")
    print(f"  Raw majority accuracy: {accuracy:.3f}")
...
def fit_irt_model(votes, true_states, n_cases, n_jurors,
    ...
    with pm.Model() as model:
        ...
        if treatment is not None:
            ...
            tau_sigma = pm.HalfNormal("tau_sigma", sigma=1.0)
        ...
        # Sample posterior
        idata = pm.sample(2000, tune=2000, chains=4, target_accept=0.90,
    ...
    # Store known truth explicitly
    import xarray as xr
    n_chains = idata.posterior.dims['chain']
    n_draws = idata.posterior.dims['draw']
    idata.posterior['z'] = xr.DataArray(
        np.tile(true_states, (n_chains, n_draws, 1)),
        dims=['chain', 'draw', 'z_dim_0']
    )
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("CONDORCET JURY THEOREM BREAKDOWN ANALYSIS")
    print("="*70)
    ...
    # ========================================================================
    # SCENARIO 1: Vanilla (Ability + Difficulty only)
    # ========================================================================
    
    print("\n" + "="*70)
    print("SCENARIO 1: VANILLA IRT")
    print("="*70)
    
    votes_vanilla, t_theta, t_delta, truth_vanilla = simulate_irt_data(
        N_CASES, N_JURORS,
        true_sigma_theta=0.8,
        true_sigma_delta=0.3,
        seed=42
    )
    
    print("\nFitting model...")
    idata_vanilla, _ = fit_irt_model(votes_vanilla, truth_vanilla, N_CASES, N_JURORS)
    ...
    # ========================================================================
    # SCENARIO 2: Blocked (Add block effects)
    # ========================================================================
    
    print("\n" + "="*70)
    print("SCENARIO 2: WITH BLOCK EFFECTS")
    print("="*70)
    ...
    # ========================================================================
    # SCENARIO 3: Full (Blocks + Treatment)
    # ========================================================================
    
    print("\n" + "="*70)
    print("SCENARIO 3: WITH BLOCKS + TREATMENT")
    print("="*70)
    ...
    print("="*70)
    print("\nIf accuracy is still near chance, check:")
    print("1. Are the generated votes actually informative? (check raw majority)")
    print("2. Is theta >> delta in magnitude? (if yes, all cases become easy)")
    print("3. Are posterior samples reasonable? (check parameter recovery)")
    print("="*70)
