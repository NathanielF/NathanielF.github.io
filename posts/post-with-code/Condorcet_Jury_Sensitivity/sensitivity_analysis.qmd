...
To understand why, we must look into how large organisations "see" the world.
...
- Consider the Algorithmic Horizon: Reflect on how AI-assisted work may 
  represent a new—and uniquely scalable—mechanism for epistemic homogenization.
:::
...
Both calculations are answering related but distinct questions. The former highlights how accuracy should scale under the model; the latter shows how it does scale when individual votes are explicitly realized. We will use both the posterior predictive checks and the posterior implications below. 
...
The parameter recovery shows reduced clustering on individual competence around the block effects. But we also see a straightline in the lower difficulty cases. This is related to the high level of competence making easier cases almost certainly correctly identified. The organization becomes "stuck in its own success." Because it is so good at the (easy) cases, it stops valuing the "Independent Friction" ($\tau$) needed to solve the hard ones. The flat line represents a state of optimal sub-optimization: you are 100% accurate on the trivial, which masks the fact that you have lost the ability to scale your intelligence on the complex. We can see this further by looking at the correlations. 
...
The treatment ($\tau$) successfully reduces the correlation by inducing differential individual competence. The heatmap should shows that the dark squares of the Blocked Model have been "shattered" into a lighter, more fragmented pattern. But surprisingly, the accuracy curve remains flat.
...
- The Full Model shows that even Expert Alignment creates a "competence trap" where you are too fixated to be truly intelligent.
