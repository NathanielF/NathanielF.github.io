...
This model structure cannot be evaluated against the true states within the model as the truth is not baked into the the likelihood as before. 
...
We see that the modelling now derives a different profile for the inferred difficulty of each problem case. The differences imply that the model without ground truth generally over estimates some easy cases and under estimates the genuinely hard cases. But there is a good degree of overlap in the cases where majority view is accurate.
...
We can compare how the accuracy of the majority evolves again by using posterior predictive sampling, to gauge the two models. Given the overlap in difficulty estimation we should expect similar cumulative accuracy.
...
Interestingly, the accuracy with `full_no_truth` model is slightly higher than we saw with the `full` model. It's not substantially different and likely the `full` model is overfitting to account for idiosyncracies of observed behaviour. However in both cases the accuracy growth plateaus. Diving a bit deeper, we can see how this is driven, oddly, by group based errors _on the easier cases_. 
...
The effects of the block have blinded the organisation and locked into error on the objectively easier cases. The easier cases enter into our skill equation as a subtraction effect. 
...
If PowerPoint was a constraint on presenting thought, large language models are something more. Both encompassing and enticing, they have so much promise but there are hints of peril.
