...
Choices are made as we step down the garden of forking paths in the course of any analysis. We risk self-delusion and foolish misstep if we cling too strongly to an arbitrary path. But we risk impoverished and inadequate theory if we refuse to explore more paths. In this post we will elaborate the choices related to analysing psychometric survey data. Psychometric phenomena are inherently complex and imprecisely measured. Attention is needed to test the assumptions used to analyse this species of data, for these models highlight a contentious aspect of the design choices made in statistical inference. There is a creativity in model development __and__ a requirement for rigor when we structure the inferences derivable from our model. We show how these confirmatory and exploratory modes of inference play-out in the Confirmatory Factor Analysis with Structural equations in PyMC under the modern Bayesian workflow. 
:::
