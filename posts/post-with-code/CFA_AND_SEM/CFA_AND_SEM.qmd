...
But as we've seen above the cleanest way to interpret almost any regression model is to understand the model design in terms of the marginal effects on the outcome scale. These are just contrasts. All statistical models are in some sense focused on finding a structure that licenses an inference about some contrast of interest between the levels of some observe variable and the implied outcomes. In this way we want to include as much structure in our model that would induce the status of conditional exchangeability between the units of study across such group contrasts. This notion of conditional exchangeability is inherently an epistemic notion. We believe that our model is apt to induce the status of conditional exchangeability such that e.g. the people in our survey have no systematic difference which biases the measured differences. The implied differences in some marginal effect (while holding all else fixed) is a "fair" representations the same counterfactual adjustment in the population. The model implied difference is a proxy for the result possible future interventions and as such merits our attention in policy design.

With this in mind we now turn to types of model architecture that open up new expressive possibilities and offer a way to think about the relationships between observed data and latent factors that drive the observed outcomes. This greater expressive power offers different routes to inducing patterns of conditional exchangeability. In particular the view here is that we ought to be deliberate in how we structure the models to license the plausibility of the exchangeability claim. 
