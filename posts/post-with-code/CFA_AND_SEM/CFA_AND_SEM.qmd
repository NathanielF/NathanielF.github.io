...
Choices are made as we step down the garden of forking paths in the course of any analysis. We risk self-delusion and foolish misstep if we cling too strongly to an arbitrary path. Maybe equally problematic, we risk impoverished and inadequate theory if we refuse to explore more paths. In this post we will elaborate this tension when analysing psychometric survey data. Psychometric phenomena are inherently complex and imprecisely measured. Care is needed to test the assumptions used to analyse this species of data, for these models highlight a contentious aspect of the design choices made in statistical inference. There is a creativity in model development __and__ a requirement for rigor when we structure the inferences derivable from our model. We show how these confirmatory and exploratory modes of inference play-out in the Confirmatory Factor Analysis with Structural equations in PyMC under the modern Bayesian workflow. 
:::
...
We will examine a smattering of choices available in the analysis of psychometric survey data. First stepping through these analysis routines using `lmer`, `lavaan` before switching to `PyMC` to demonstrate how to fit Confirmatory Factor Analysis models and Structural Equation Models in a Bayesian fashion using a probabilistic programming language.Throughout we'll draw on Levy and Mislevy's [_Bayesian Psychometric Modeling_](https://www.taylorfrancis.com/books/mono/10.1201/9781315374604/bayesian-psychometric-modeling-roy-levy-robert-mislevy).m

### The Data

The data is borrowed from work [here](https://methodenlehre.github.io/SGSCLM-R-course/cfa-and-sem-with-lavaan.html#structural-equation-modelling-sem) demonstrating SEM modelling with `Lavaan`. We'll load it up and begin some exploratory work. First let's look at the data. 
...
$$ p(x_{i}.....x_{n} | ksi, \Psi, \tau, \Lambda) \sim Normal(\tau + \Lambda\cdot ksi, \Psi) $$
...
Then we can plot the familiar heatmaps from above highlighting the comparable accuracy of the Bayesian model fit. 
...
```
The regrssion effects seem well sampled and show the strength of relationships between social self-efficacy scores and reported life-satisfaction
...
Note that we've defined these effects as point estimates on the mean realisations of the regression coefficients. But we could have defined these quantities in the model context and sampled the posterior distribution too. However, the model was already complex enough that we kept it simpler for the clarity of exposition.

Finally we can plot the residual variance-covariance for the Bayesian estimate of Structural equation model we hypothesised. 
...
The overall model fit seems quite reasonable, but __notably__ distinct from the fit of the simpler measurment model. In this way the flexibility of the bayesian estimation strategy is not as constrained as the MLE variance-covariance estimation strategy and we can learn of meaningful interactions in a richer more theoretically compelling model while still calibrating against the observed data.
