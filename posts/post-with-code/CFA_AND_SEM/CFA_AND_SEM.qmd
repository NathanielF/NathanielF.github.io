...
Choices are made as we step down the garden of forking paths in the course of any analysis. If in the process of frantic search we cling too strongly to the first feasible path, we risk self-delusion from directionless flailing. Equally problematic, we risk impoverished and inadequate theory if we refuse to explore more paths. In this post we will elaborate this explore/exploit tension when analyzing psychometric survey data. Psychometric models in particular highlight this contentious aspect of statistical inference. In psychometric theory we must vary the hypothesised structure to our manifest and latent variables which cannot be done whimsically. There is a trial and error kind of creativity in model development **and** a requirement for rigor when we structure the inferences derivable from our model. We show how these confirmatory and exploratory modes of inference play-out in the Confirmatory Factor Analysis with Structural equations in PyMC under the modern Bayesian workflow.
:::
...
![Structural Equation Models](sem_model.svg) The main point to emphasise here is the expressive nature of the modelling paradigm. Our ability to construct and estimate different chains of dependence allows us to test evaluate the direct and indirect effects due to the different "drivers" of life-satisfaction.
...
```
The structural model is marginally worse by measures of overall fit to the data, but this summary skips over the variety of questions we can answer with this new structure. The broad fidelity to observed datas seen in the posterior predictive checks and the variance covariance residuals suggests that we ought to be willing to take this hit to overall performance if we can answer questions left unanswered by the measurement model.  
