...
::: {.callout-info}
Choices are made as we step down the garden of forking paths in the course of any analysis of data. We risk self-delusion and foolish misstep if we cling too strongly to an arbitrary path and we risk impoverished and inadequate theory if we refuse to explore many paths. In this post we will elaborate the choices related to analysing psychometrics surveys. Psychometric phenomena are inherently complex and imprecisely measured. Special attention is needed for the assumptions built into the methodologies used to analyse this species of data. Additionally these models highlight another aspect of the design choices made in statistical inference. There is a creativity in model development __and__ a requirement for rigor when we deliberately and explicitly structure the inferences derivable from our model. We show how these confirmatory and exploratory modes of inference play-out in the Confirmatory Factor Analysis with Structural equations in PyMC under the modern Bayesian workflow. 
:::
