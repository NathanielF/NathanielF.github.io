...
::: {.callout-warning}
Statistical models are designed. Choices are made as we step down the garden of forking paths in the course of any analysis. This risks our self-delusion and mistaken inference if cling too strongly to an arbitrary path. We intend to elaborate the choices related to analysing and understanding the data elicted through psychometrics surveys. Psychometric phenomena are inherently complex and imprecisely measured. Special attention needs to the assumptions built into the methodologies used to analyse this species of data, but these models also highlight another aspect of the design choices made in statistical inference. There is a creativity available in design strategies which help us deliberately and explicit structure the conditions of statistical inference within these models. We show how these modes of inference play-out in the Confirmatory Factor Analysis with Structural equations in PyMC. 
:::


## Measurment and Measurement Constructs

Science is easier when there is a recipe. When there is some formula to follow,  a procedure to adopt or routine to ape, you can out-source the responsibility for theory construction and methodological justification. One egregious pattern in this vein masks implausible nonsense with much vaunted mathematical gloss of "statistical significance". Seen from 1000 feet, this is not surprising. Lip-service is paid to the idea of scientific method and we absolve ourselves of the requirement for principled justification and substantive argument. Evidence should be marshaled in service to argument, and it's an absurd pretense to claim that data speaks for itself in this argument.
...
We can also compare models based on their global measures of model fit giving some indication of whether parameter specifications improve or reduce fidelity with reality. 
...
![Indirect Effects and Total Effects Model](indirect_sem.svg)
