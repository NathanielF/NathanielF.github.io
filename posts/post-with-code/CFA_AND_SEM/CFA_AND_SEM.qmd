...
heat_df = cov(df[,  drivers]) |> as.matrix() |> melt()
...
heat_df |> mutate(value = round(value, 2)) |>  ggplot(aes(x = x, y = y, fill = value)) +
 ...
 scale_fill_gradient2(
    ...
    mid = 'white',
    low = 'firebrick2'
  ) + theme(axis.text.x = element_text(angle=45)) + ggtitle("Sample Covariances", "Observed Measures")
...
# Measurement model
SUP_Parents =~ sup_parents_p1 + sup_parents_p2  + sup_parents_p3
SUP_Friends =~ sup_friends_p1 + sup_friends_p2 + sup_friends_p3 + se_social_p1
SE_Academic =~ se_acad_p1 + se_acad_p2 
SE_Social =~ se_social_p1 + se_social_p2 
LS  =~ ls_p1 + ls_p2 
"
...
modindices(fit_mod_1, sort = TRUE, maximum.number = 6, standardized = FALSE, minimum.value = 5)
...
modelplot(fit_mod_sem, coef_omit='=~')
