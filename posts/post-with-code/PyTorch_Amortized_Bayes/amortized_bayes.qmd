...
def plot_heatmap(data, title="Correlation Matrix"):
...
plot_heatmap(sample_df.corr() - reconstructed_df.corr())
...
pd.DataFrame(az.summary(idata, var_names=['chol_corr'])['mean'].values.reshape((12, 12)))
...
axs[0].hist(az.extract(idata['posterior_predictive'])['likelihood'].mean(dim=('obs'))[1, :].values)
...
pd.DataFrame(az.summary(idata_missing, var_names=['chol_corr'])['mean'].values.reshape((12, 12)))
...
validation_data = workflow.simulate(100)
...
```{python}
#history = workflow.fit_offline(data=training_data, num_batches_per_epoch=500, epochs=10, validation_data=validation_data, batch_size=64);


history = workflow.fit_online(num_batches_per_epoch=1000, epochs=10, validation_data=validation_data, batch_size=32);
...
```{python}
post_draws = workflow.sample(conditions={'y': sample_df.values}, num_samples=num_samples)

post_draws
expected_mus = post_draws['mu'][0, :, :].mean(axis=0)
L = np.zeros((12, 12))
L[np.tril_indices_from(L)] = post_draws['sigma'][0, :, :].mean(axis=0)

expected_sigma = L @ L.T

expected_sigma - cov_df
