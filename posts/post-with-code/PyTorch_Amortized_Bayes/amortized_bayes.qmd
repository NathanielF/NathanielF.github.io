...
We plot the estimates parameters here. We might worry about the parameter identification here, but since we're not using real data the point is a little moot as we haven't properly encoded the regression structure in our simulation data. But even so, it seems we can recover and expected positive impact of constructive thought strategies on job-satisfaction. 
