...
This analysis explores how different modeling choices; deep generative models versus structured Bayesian inference—handle reconstruction, imputation, and inductive bias. Along the way, we confront a central difficulty: in high-dimensional latent spaces, it’s easy to be wrong in convincing ways. Learning to “fill in the blanks” isn’t just a technical challenge - it’s a philosophical one.
...
def plot_metric_recovery(fit_vae, test_loader):
    recon_df = pd.DataFrame(fit_vae.generate(test_loader.dataset.x.shape[0]), columns=FEATURE_COLUMNS)

    test_df = pd.DataFrame(test_loader.dataset.x, columns=FEATURE_COLUMNS)
...
plot_metric_recovery(fit_vae_missing_5000, test_loader_5000)
