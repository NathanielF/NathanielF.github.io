...
def make_B(priors=[0, .5], centred=True):
    mu = pm.Normal('mu_betas', 0, .5, dims='paths')
    if not centred:
        beta_std = pm.Normal("beta_trt_std", 0, .5, dims="paths")

        # non-centered transform
        raw = pm.Normal("raw_betas", 0, .2, dims="paths")
        sigma = pm.HalfNormal("beta_sigma", 1.0, dims="paths")
        coefs = pm.Deterministic("betas", mu + raw * sigma)
    else: 
        coefs = mu
...
with pm.Model(coords=coords) as sem_model_clean:
    ...
    lhs = I - coefs_  # (latent_dim, latent_dim)
    ...
    # --- Measurement model: predicted observed means ---
    # Multiply latent scores by factor loadings
...
with sem_model_clean: 
    idata = pm.sample_prior_predictive()
    idata.extend(pm.sample(chains=4, tune=2000, draws=5000, target_accept=.95, random_seed=232))

```


We plot the estimates parameters here. We might worry about the parameter identification here, but since we're not using real data the point is a little moot as we haven't properly encoded the regression structure in our simulation data. But even so, it seems we can recover the positive impact of constructive thought strategies on job-satisfaction. 
